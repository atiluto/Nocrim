...
func _init() -> void:
	...
	check(clock.date_text(0)=="중원력 732년 3월 상순","murim start")
	check(clock.date_text(1)=="중원력 732년 3월 중순","middle segment")
	check(clock.date_text(2)=="중원력 732년 3월 하순","late segment")
	check(clock.date_text(3)=="중원력 732년 4월 상순","month rollover")
	check(clock.date_text(29)=="중원력 732년 12월 하순","year end")
	check(clock.date_text(30)=="중원력 733년 1월 상순","year rollover")
	check(clock.date_text(21,"modern")=="2027년 1월 상순","modern year rollover")
	var c=Campaign.new(); c.new_game(71)
	for day in range(1,11):
		for period in ["아침","낮","밤"]:
			check(clock.label_for(c.s)=="중원력 732년 3월 상순 · %d일 · %s" % [day,period],"daily phase sequence")
			check(c.perform("end_turn").ok,"spend one phase")
	check(clock.label_for(c.s)=="중원력 732년 3월 중순 · 11일 · 아침","thirty actions make one ten-day period")
	c.new_game(72); c.s.turn=4; c.s.ap=2; c.s.erase("calendar_cycle_offset")
	check(clock.label_for(c.s)=="중원력 732년 4월 상순 · 1일 · 낮","old turn/ap migrate without reset")
	...
	check(murim[0].scene_status.date=="중원력 732년 3월 상순","prologue murim start")
	var last: Dictionary=script.beats[-1].scene_status.calendar
	c.complete_prologue(last)
	check(clock.cycle_for(c.s)==2 and c.s.calendar_day==1 and c.s.ap==3,"next morning after prologue")
