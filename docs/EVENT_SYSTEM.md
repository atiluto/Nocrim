...
176개 개별 사건과 별도의 연쇄 설계 4개를 원문과 함께 등록했다. 개별 사건 중 **134개는 기본 진행 경로, 23개는 부분 진행, 19개는 미구현**이다. 상세 목록·사유는 [EVENT_CATALOG.md](EVENT_CATALOG.md). `ready`는 현재 등록한 기본 실행 경로에 미해결 분기가 없다는 뜻이며, 원문의 모든 장기 확장·미술·대사를 완성했다는 뜻은 아니다. 부분 진행 사건은 가능한 선택만 실행하고 미구현 선택은 흐리게 표시한다.
...
마영란 전용 그림은 현재 자산에 없어 등록하지 않았다. 지역 대화는 기존 배경·음원·대화창으로 진행하며 인물 이미지는 비어 있다. 다른 역할의 NPC나 죽은 인물 그림으로 대신하지 않았다. 기존 프롤로그 캐스팅과 전투는 유지한다.
...
| `scenario/events/chapters/<사건 ID>/*.md` | 한 ID = 한 화면인 실제 대사, 60개 파일·804개 화면 |
