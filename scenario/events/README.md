...
- 기존 사건 순서/선택/보상/조건: `catalog.json`. 장소연·하령화 루트와 SJ-C04·DN-C02·CH-C02는 **routes/의 전용 루트 MD 안 `route-json`**이 실행 원본이며 catalog보다 우선한다. `nodes`에서 MD 대사 ID 목록과 다음 노드를 연결한다.
...
마영란 전용 스프라이트 6종과 대사별 표정을 등록했다. 장소연·하령화는 각각 3종을 전용 대사·동행·연애/동료 루트에 연결했다. 시작 조건과 편집은 [전용 루트 안내](routes/README.md)를 참고한다. 그림은 사용자 요청대로 **흰 배경**이며 직접 배경을 제거하기 전에는 게임에서도 흰 사각형이 보인다. [히로인 이미지·표정 안내](../../docs/HEROINE_ART.md)를 참고한다. 기존 NPC를 다른 역할로 재활용하지 않는다. 미구현 선택은 `pending:true`로 남아 있으며 대사 몇 줄만 바꾼다고 그 기능까지 생기지는 않는다.
